...
export async function POST(request){
    try {
        //{code, contactPerson, contactPersonPhone, email, name, notes, phone, physicalAddress, terms, isActive, profileImageUrl, products, landSize, mainCrop, userId}
        const farmerData = await request.json();
        const newFarmerProfile = await db.farmerProfile.create({
            data: {code :farmerData.code,
        ...
        console.log("New Farmer" , newFarmerProfile)
...
export async function GET (request){
    try{
        ...
        console.log(error)
        return NextResponse.json({
            message: "Failed to Fetch Farmer's Profiles",
            error
        }, {status:500})
    }
}